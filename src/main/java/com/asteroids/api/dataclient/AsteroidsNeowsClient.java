...
public class AsteroidsNeowsClient extends RestClient {
  ...
  public AsteroidsNeowsClient(
      @Value("${nasa-api.url}") final String url,
  ...
  public JsonNode getAsteroids(String startDate, String endDate) {
    try {
      return webClient
          .get()
          .uri(
              "/neo/rest/v1/feed?start_date={startDate}&end_date={endDate}&api_key={apiKey}",
              startDate,
              endDate,
              apiKey)
          ...
          .onStatus(
              ...
              response -> {
                log.error(
                    "Unable to get asteroids from NASA API. Response: {}", response.toString());
